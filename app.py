...
class CalendarAssistant:
    ...
    async def _detect_time_conflict_async(self, date: str, start_time: str, end_time: str) -> bool:
        ...
        try:
            ...
            date_url = f"https://calendar.google.com/calendar/u/0/r/day/{date_obj.year}/{date_obj.month}/{date_obj.day}"
            await self.page.goto(date_url, wait_until="domcontentloaded")
            await asyncio.sleep(3)

            target_start = datetime.strptime(start_time, "%I:%M %p").time()
            target_end = datetime.strptime(end_time, "%I:%M %p").time()

            events = await self.page.query_selector_all('[role="gridcell"] [aria-label*="Event"]')
            for event in events:
                label = await event.get_attribute("aria-label")
                if not label:
                    continue
                time_match = re.search(r'(\d{1,2}:\d{2}\s[APM]{2})\s*-\s*(\d{1,2}:\d{2}\s[APM]{2})', label)
                if time_match:
                    event_start = datetime.strptime(time_match.group(1), "%I:%M %p").time()
                    event_end = datetime.strptime(time_match.group(2), "%I:%M %p").time()
                    if not (target_end <= event_start or target_start >= event_end):
                        logger.info(f"Conflict detected: {event_start}-{event_end}")
                        return True
            return False
        except Exception as e:
...
class VoiceParser:
    def parse(self, text: str) -> Dict[str, str]:
        ...
        start_time = "9:00 AM"
        end_time = "10:00 AM"
        time_range_match = re.search(r'(\d{1,2})(?::(\d{2}))?\s*(?:to|-|until)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)', text_lower)
        if time_range_match:
            ...
            start_time = f"{hour1}:{minute1:02d} {period}"
            end_time = f"{hour2}:{minute2:02d} {period}"
...
def handle_connect():
    ...
    if calendar.first_connection:
        msg = ("Hello! I am your Voice Calendar Assistant. "
               ...
               "Please say something like: 'Schedule a meeting with the company CEO tomorrow'.")
